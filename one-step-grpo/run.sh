...
cd /mnt/shared-storage-user/yangjingyi/EOSER-ASS-RL/one-step-grpo
...
LOGDIR=/mnt/shared-storage-user/yangjingyi/EOSER-ASS-RL/one-step-grpo/logs
SAVEDIR=/mnt/shared-storage-user/yangjingyi/EOSER-ASS-RL/one-step-grpo/checkpoints
mkdir -p $LOGDIR
mkdir -p $SAVEDIR
echo $LOGDIR
echo $SAVEDIR
...
DATASET="countdown"
RUN_NAME=one_step_grpo_${DATASET}_128_64_64
MODEL_PATH=GSAI-ML/LLaDA-8B-Instruct
NUM_ITER=8 # number of policy gradient inner updates iterations

accelerate launch \
    --config_file accelerate.yaml \
    --main_process_port 12346 one_step_grpo_train.py \
