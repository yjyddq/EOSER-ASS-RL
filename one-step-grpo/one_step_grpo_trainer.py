...
class FlowGRPOTrainer(GRPOTrainer):
    ...
    def _get_per_token_logps(self, model, input_ids, logits_to_keep):
        ...
        x = torch.full((batch_size, seq_len), self.args.mask_id, dtype=torch.long).to(model.device) # append the mask tokens
        x[:, :prompt_length] = prompt_ids.clone()

        logits = self.get_logits(
            model, x, prompt_index, self.args.cfg_scale, self.args.mask_id
        )  # [B, L, V]
