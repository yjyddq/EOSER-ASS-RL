...
class CJGRPOTrainer(GRPOTrainer):
    ...
    def training_step(
        ...
        total_loss = torch.tensor(0.0, device=completion_ids.device)
        mean_kl = torch.tensor(0.0, device=completion_ids.device)
        clip_ratio = torch.tensor(0.0, device=completion_ids.device)
    ...
    def ass_sampling(self, logits, x0, remasking, dtype, step_idx=None, total_steps=None, eos_min_gamma=None, eos_max_gamma=1.0):
        assert remasking == "low_confidence", f"Expected remasking == low_confidence in sampling"
        p = F.softmax(logits.to(dtype), dim=-1)
        x0_p = torch.squeeze(
            torch.gather(p, dim=-1, index=torch.unsqueeze(x0, -1)), -1
        ...
        # Soft, step-dependent EOS suppression
        eos_id = self.processing_class.eos_token_id
        min_fac = eos_min_gamma if eos_min_gamma is not None else getattr(self.args, "eos_min_gamma", 0.01)
        max_fac = eos_max_gamma
        if step_idx is not None and total_steps is not None and total_steps > 1:
    ...
    def reversed_process(
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            current_pos = prompt.shape[1]
            ### Record unmask positions of each step ###
                    ...
                    with torch.cuda.amp.autocast(enabled=self.args.fp16):
                        ...
                        if remasking == "low_confidence":
                            x0_p = self.ass_sampling(
                                    ...
                                    step_idx=i,
                                    total_steps=steps,
                                    eos_min_gamma=getattr(self.args, "eos_min_gamma", 0.01),
